...
from src.job_api import fetch_linkdin_jobs, fetch_naukari_jobs

st.set_page_config(page_title="Job Recommender", layout="wide")
st.title("AI Job Recommender 😍")
st.markdown("upload your resume and get job recommendations based on your skills and experience from Linkdin & Naukari")
...
if uploaded_file:
    ...
    with st.spinner("Finding skill Gaps..."):
        gaps = ask_openai(f"Analyze this resume and highlight missing skills, certifications and experiences needed for better job opportunities: \n\n{resume_text}", max_token=400)

    with st.spinner("Creating future roadmap..."):
        roadmap = ask_openai(f"based on this resume, suggest the future roadmap to improve this persons career prospects(skill to learn, certification needed, industry exposure): \n\n{resume_text}", max_token=400)
    ...
    if st.button("🔎Get Job Recommendations"):
        ...
        with st.spinner("Fetching jobs from LinkedIn and Naukri..."):
            linkedin_jobs = fetch_linkdin_jobs(search_keywords_clean, rows=60)
            naukri_jobs = fetch_naukari_jobs(search_keywords_clean, rows=60)
